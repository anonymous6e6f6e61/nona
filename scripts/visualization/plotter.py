...
plot_types = ['lr_overheads', 'cl_overheads', 'lr_static', 'ri_static',
              'cl_static', 'lr_dynamic', 'ri_dynamic', 'synthetic']
...
parser.add_argument(
    'plot',
    choices=plot_types,
    help='Choose the type of plot: lr_static, cl_static, lr_dynamic, synthetic'
)
...
def plot_static(data_df, output_name, show_popup=False):
    ...
    axes[3].legend(handles=custom_legend_elements, handlelength=1.0, handletextpad=0.5, loc="lower center")
...
def plot_dynamic(folder, output_name, show_popup=False):
    ...
    for transition in range(len(addition_durations[1])):
        addition_averages.append(np.mean([addition_durations[rep][transition] for rep in reps]))
        addition_stds.append(np.std([addition_durations[rep][transition] for rep in reps]))
        removal_averages.append(np.mean([removal_durations[rep][transition] for rep in reps]))
        removal_stds.append(np.std([removal_durations[rep][transition] for rep in reps]))
        flink_averages.append(np.mean([flink_durations[rep][transition] for rep in reps]))

    x_tick_positions = list(range(1, 1 + len(addition_durations[1])))

    addition_labels = [f"+Q{i}" for i in x_tick_positions]
    axes[0].set_title("Transition durations (ms)")
    ...
    for p, a, f in zip(axes[0].patches, addition_averages, flink_averages):
        axes[0].text(
            p.get_x() + p.get_width() * 0.85,  # x-position
            p.get_height() * 0.99,  # y-position
            f'{int(a - f)}ms ({int((a - f) / a)}%)',
    ...
    axes[0].text(0.91, 0.05, "Additions", transform=axes[0].transAxes, fontsize=12,
                 verticalalignment='bottom', horizontalalignment="center",
                 bbox={"boxstyle": 'round',
                       "facecolor": 'whitesmoke',
                       "edgecolor": "grey"}
                 )
    axes[1].text(0.91, 0.05, "Removals", transform=axes[1].transAxes, fontsize=12,
                 verticalalignment='bottom', horizontalalignment="center",
                 bbox={"boxstyle": 'round',
                       "facecolor": 'whitesmoke',
                       "edgecolor": "grey"}
...
def plot_synthetic(folder, show_popup=False):
    ...
    output_name = "removal_times_synthetic_figure"
